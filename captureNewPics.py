    ...
    def test_screenShot(self):
        ...
        if len(param) == 4:
            ...
            if not os.path.exists('images/new/' + param[1]):
                os.makedirs('images/new/' + param[1])
            if not os.path.exists('images/new/' + param[1] +'/' + param[2] + ' ' + param[3]):
                os.makedirs('images/new/' + param[1] +'/' + param[2] + ' ' + param[3])
        else:
            if not os.path.exists('images'):
                os.makedirs('images')
            if not os.path.exists('images/new/'):
            ...
            if not os.path.exists('images/new/' + param[1]):
                os.makedirs('images/new/' + param[1])
            if not os.path.exists('images/new/' + param[1] +'/' + param[2]):
                os.makedirs('images/new/' + param[1] +'/' + param[2])
        ...
        for i in range(5):
            ...
            time.sleep(3.0)
            self._browser.save_screenshot('images/new/' + param[1] + '/' + param[2] +'/' + name)
